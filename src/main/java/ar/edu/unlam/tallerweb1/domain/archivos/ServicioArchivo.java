...
public class ServicioArchivo implements IServicioArchivo {
    ...
    private final Long MAX_SIZE_UPLOAD = 1000000L;
    private final int MAX_UPLOADS_FILES = 4;
    ...
    public String encodeImage(MultipartFile multipart) {
        ...
        try {
            ...
            this.sizeVerify(multipart);
            encodedString = this.bytesToBase64(multipart.getBytes());
    ...
    private void sizeVerify(MultipartFile multipart) {

        if(multipart.isEmpty()) throw new EmptyFileException();

        if(multipart.getSize() > MAX_SIZE_UPLOAD) throw new MaxSizeFileException("Las imagenes no pueden superar " + MAX_SIZE_UPLOAD/1000000 + " Mb");
    }
    ...
    @Override
    public void validarArchivos(MultipartFile[] files) {
        if(files.length > MAX_UPLOADS_FILES) throw new FileValidationException("No se permite cargar mas de " + MAX_UPLOADS_FILES + " archivos");

        for (MultipartFile file : files) {
            this.sizeVerify(file);
        }
    }
